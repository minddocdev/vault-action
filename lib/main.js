...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const authMethod = types_1.AuthMethods.github; // TODO - Support other auth methods
            const secretsEngine = loadYaml('secretEngine', { required: false }) || {
                name: types_1.SecretsEngines.KV,
                config: {
                    path: '/kv',
                    version: 2,
                },
            };
            if (secretsEngine.name !== types_1.SecretsEngines.KV) {
            ...
            yield core.group(`Retrieve Vault Secrets using ${secretsEngine.name} engine`, () => kv_1.importKV2Secrets(vaultClient(`${url}/v1${secretsEngine.config.path}`, headers), secrets));
        }
        catch (error) {
